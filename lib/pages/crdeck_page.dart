...
class _CrDeckPage extends State<CrDeckPage> {
  ...
  Future<void> loadAllCardsFromServer() async {
    await httpGet(apiEndpoint, {}).then((response) {
      if (response.statusCode == 200) {
        setState(() {
          allCards = (jsonDecode(response.body) as Map<String, dynamic>)["items"].map<CrDeckCard>((item) =>
              CrDeckCard.fromJson(item)).toList();
        });
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  t.tools.crdeck.error.failed_to_load_cards_from_server(errorCode: response.statusCode)
              ),
            ),
          );
        }
  ...
  Future<void> loadSelectedCards() async {
    ...
    final savedCardsIds = prefs.getStringList(SHARED_PREFERENCES_TOOL_CRDECK_SELECTEDCARDSIDS);
    if (savedCardsIds == null) {
      selectedCards = List.from(allCards);
    } else {
      final savedCardsIdsInt = savedCardsIds.map((id) => int.tryParse(id) ?? -1).where((id) => id != -1).toList();
      selectedCards = savedCardsIdsInt.map((cardId) => allCards.firstWhere((card) => card.id == cardId)).toList();
    }
  }

  Future<void> saveSelectedCards() async {
    final prefs = await SharedPreferences.getInstance();
    final selectedCardsIds = selectedCards.map((card) => card.id.toString()).toList();
    await prefs.setStringList(SHARED_PREFERENCES_TOOL_CRDECK_SELECTEDCARDSIDS, selectedCardsIds);
  }

  Future<List<CrDeckCard>?> showImportCardsFromPlayerProfileDialog(BuildContext context) async {
          ...
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                if (playerTag.isEmpty || playerTag.length < 7 || playerTag.length > 13) {
                  Navigator.of(context).pop();
                  showOkTextDialog(
                      context,
                      t.tools.crdeck.error.invalid_tag,
                ...
                try {
                  final apiUrl = "$apiEndpoint/${Uri.encodeComponent(playerTag)}";
                  final response = await httpGet(apiUrl, {});
                  if (response.statusCode == 200) {
                    final importedCards = (jsonDecode(response.body) as Map<String, dynamic>)["items"].map<CrDeckCard>((item) => CrDeckCard.fromJson(item)).toList();
                    if (mounted) {
                      Navigator.of(context).pop();
                    ...
                    if (mounted) {
                      ...
                      showOkTextDialog(
                          context,
                          t.generic.error,
                          t.tools.crdeck.error.failed_to_import_cards_from_player_profile(errorCode: response.statusCode)
                      );
                    }
                  }
                } catch (e) {
                  if (mounted) {
                    ...
                    showOkTextDialog(
                        context,
                        t.generic.error,
                        t.tools.crdeck.error.an_error_occurred_while_importing_cards
                    );
                  }
                }
              },
  ...
  void randomizeDeck() {
    var selectedCardsNoLocked = selectedCards.where((card) => !lockedDeckCards.contains(card)).toList();
    if (selectedCardsNoLocked.length < maxDeckCards - lockedDeckCards.length) {
      showOkTextDialog(
          context,
          t.tools.crdeck.error.not_enough_cards_selected,
          t.tools.crdeck.error.please_select_at_least_x_more_cards(numberOfCards: (maxDeckCards - lockedDeckCards.length))
      );
      return;
    }
    setState(() {
      final availableCards = selectedCards.where((card) => !lockedDeckCards.contains(card)).toList();
  ...
  void addCardToDeck(CrDeckCard card) {
    if (showedDeckCards.length >= maxDeckCards) {
      showOkTextDialog(
          ...
          t.tools.crdeck.error.you_can_only_add_up_to_x_cards_to_the_deck(numberOfCards: maxDeckCards)
      );
      return;
    }
    if (showedDeckCards.contains(card)) {
      showOkTextDialog(
          context,
          t.tools.crdeck.error.card_already_in_deck,
  ...
  Future<void> openDeckInClashRoyale() async {
    if (showedDeckCards.length < maxDeckCards) {
      showOkTextDialog(
          ...
          t.tools.crdeck.error.please_add_x_more_cards_to_the_deck_before_opening_it_in_clash_royale(numberOfCards: maxDeckCards - showedDeckCards.length)
      );
      return;
    }
  ...
  Future<void> shareDeck() async {
    if (showedDeckCards.length < maxDeckCards) {
      showOkTextDialog(
          context,
          t.tools.crdeck.error.incomplete_deck,
          t.tools.crdeck.error.please_add_x_more_cards_to_the_deck_before_sharing_it(numberOfCards: maxDeckCards - showedDeckCards.length)
      );
      return;
    }
    String deckCode = showedDeckCards.map((card) => card.id).join(";");
          ...
          body: SafeArea(
            child: isLoading
                ...
                child: Padding(
                  padding: EdgeInsets.all(16.0),
                  child: Text(
                    t.tools.crdeck.error.please_check_your_internet_connection,
                ...
                children: [
                          ...
                          : GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return GestureDetector(
                                onTap: () {
                                  setState(() {
                                    if (lockedDeckCards.contains(card)) {
                                      lockedDeckCards.remove(card);
                                    } else {
                      ...
                      children: [
                        Expanded(
                          child: FilledButton(
                            ...
                            child: Text(t.tools.crdeck.generate_new_deck),
                          ),
                        ),
                        SizedBox(width: 4),
                        IconButton(
                          ...
                          tooltip: t.tools.crdeck.open_deck_in_clash_royale,
                        ),
                        IconButton(
                  ...
                  Padding(
                    ...
                    child: TextField(
                      ...
                      onChanged: (value) {
                        setState(() {
                          filteredSearchCards = allCards.where((card) => card.name.toLowerCase().contains(value.trim().toLowerCase())).toList();
                        });
                      },
                    ),
                  ),
                  Padding(
                    padding: EdgeInsets.only(left: 2.0, right: 2.0, bottom: 2.0, top: 0.0),
                    child: SizedBox(
                      width: double.infinity,
                      child: Row(
                        ...
                        children: [
                          IconButton(
                              ...
                              tooltip: t.tools.crdeck.import_cards_from_player_profile,
                              onPressed: () async {
                                final cards = await showImportCardsFromPlayerProfileDialog(context);
                                if (cards != null) {
                                  setState(() {
                                    selectedCards = allCards.where((card) => cards.any((importedCard) => importedCard.id == card.id)).toList();
                                  });
                                }
                              }
                          ),
                          Row(
                            ...
                            children: [
                              IconButton(
                                ...
                                tooltip: t.tools.crdeck.deselect_all_cards,
                              ),
                              IconButton(
                                ...
                                tooltip: t.tools.crdeck.select_all_cards,
                              ),
                            ],
                          ),
                        ],
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            leading: Container(
                              foregroundDecoration: selectedCards.contains(card)
                                  ? null
                                  : BoxDecoration(
                                color: Colors.grey,
                            ...
                            subtitle: Text(t.tools.crdeck.x_elixirs(elixirs: card.elixirCost ?? '?'))
                          ),
                        );
                      },
                    ),
