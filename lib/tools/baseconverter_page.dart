...
class _BaseConverterPage extends State<BaseConverterPage> {
    ...
    switch (key) {
      ...
      default:
        final int? parsedValue =
            int.tryParse(_controller.text + key, radix: _getRadix(currentBase));
  ...
  String _getConvertedValue(String mode) {
    final int? parsedValue =
        int.tryParse(_controller.text, radix: _getRadix(currentBase));
  ...
  void checkInputIsValid(String? base) {
    if (_controller.text.isNotEmpty) {
      final int? parsedValue =
          int.tryParse(_controller.text, radix: _getRadix(base ?? 'DEC'));
      if (parsedValue == null) {
