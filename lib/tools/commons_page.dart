...
class _CommonsPage extends State<CommonsPage> {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.containsKey('query') && data['query'].containsKey('pages')) {
          final Map<String, dynamic> pages =
              Map<String, dynamic>.from(data['query']['pages']);
          List<CommonsFileInfo> fetchedFiles = [];
          for (var page in pages.values) {
            fetchedFiles
                .add(CommonsFileInfo.fromJson(Map<String, dynamic>.from(page)));
          }
          setState(() {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text('${t.generic.error}: ${response.statusCode}')),
          );
          setState(() {
            mediaFiles = [];
          });
  ...
  Future<void> showImagePreviewDialog(BuildContext context, String imageUrl,
      {bool isSvg = false}) async {
  ...
  void showMainDialog(CommonsFileInfo file) {
    ...
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            title: Text(file.cleanName.isNotEmpty
                ? getContentOfHtmlTags(file.cleanName)
                : getContentOfHtmlTags(file.fileName)),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(t.tools.commons.file_information,
                      style: TextStyle(fontWeight: FontWeight.bold)),
                  Text("${t.tools.commons.mime_type} : ${file.mimeType}"),
                  Text(
                      "${t.tools.commons.file_size} : ${getFileSizeString(file.size)}"),
                  if (file.width != 0 && file.height != 0)
                    Text(
                        "${t.tools.commons.dimensions} : ${file.width} x ${file.height}"),
                  if (file.duration != null)
                    Text(
                        "${t.tools.commons.duration} : ${getFileDurationString(file.duration!)}"),
                  SizedBox(height: 16),
                  Text(t.tools.commons.file_license,
                      style: TextStyle(fontWeight: FontWeight.bold)),
                  Text(file.licenseShortName.isNotEmpty
                      ? file.licenseShortName
                      : t.tools.commons.unknown_license),
                  if (file.licenseTerms.isNotEmpty)
                    Text(getContentOfHtmlTags(file.licenseTerms)),
                  SizedBox(height: 8),
                  if (file.licenseAttribution.isNotEmpty) ...[
                    SizedBox(height: 8),
                    Text(t.tools.commons.file_by,
                        style: TextStyle(fontWeight: FontWeight.bold)),
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(t.tools.commons.error.error_downloading_file(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                t.tools.commons.error.error_download_file_check_internet)));
      }
    }
    if (mounted) {
              ...
              IconButton(
                ...
                onPressed: () {
                  List<TextButton> buttons = [
                    TextButton(
                      onPressed: () =>
                          launchUrlInBrowser("https://commons.wikimedia.org"),
