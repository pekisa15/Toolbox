...
class _SshClientPage extends State<SshClientPage> {
  ...
  Future<void> initSshClient(
      String host, String username, String password, int port) async {
    client = SSHClient(
  ...
  Future<void> connectToSsh() async {
    ...
    try {
      if (usingSshKey) {
        String? passphrase =
            passphraseController.text == "" ? null : passphraseController.text;
        await initShhClientWithKeys(host, username,
            await sshPrivateKey?.readAsString() ?? "", passphrase, port);
      } else {
      ...
      await prefs.setBool(
          SHARED_PREFERENCES_TOOL_SSHCLIENT_USINGKEY, usingSshKey);
      ...
      if (mounted) {
        showOkTextDialog(
            context, t.generic.error, t.tools.sshclient.error.unknown_error);
      }
    } finally {
                    ...
                    ? IconButton(
                        ...
                        onPressed: () {
                          if (mounted) {
                            setState(() {
                              shell?.stdin
                                  .add(Uint8List.fromList("\x7F".codeUnits));
                            });
                          }
                        },
                      )
                    : IconButton(
                        icon: Icon(usingSshKey
                            ? Icons.password_outlined
                            : Icons.key_outlined),
                        tooltip: usingSshKey
                            ? t.tools.sshclient.use_password
                            : t.tools.sshclient.use_ssh_key,
                        onPressed: () {
                          setState(() {
                                ...
                                children: [
                                  Row(
                                    children: [
                                      ...
                                      const SizedBox(width: 8),
                                      Text(
                                        t.tools.sshclient.authentication,
                                        style: Theme.of(context)
                                            .textTheme
                                            .titleMedium
                                            ?.copyWith(
                                      ...
                                      Container(
                                        ...
                                        child: Text(
                                          usingSshKey
                                              ? t.tools.sshclient.ssh_key
                                              : t.tools.sshclient.password,
                                          style: Theme.of(context)
                                              .textTheme
                                              .bodySmall
                                              ?.copyWith(
                                  ...
                                  if (usingSshKey) ...[
                                    OutlinedButton.icon(
                                      ...
                                      icon: Icon(
                                        ...
                                        color: sshPrivateKey != null
                                            ? colorScheme.primary
                                            : null,
                                      ),
                                      label: Text(
                                          t.tools.sshclient.select_private_key),
