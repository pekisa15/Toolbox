...
class _NearbyPublicTransportStopsPage
  ...
  Future<void> deletePreviousVersionDownloadedFiles() async {
    ...
    const String csvVersionFileName =
        "nearbypublictransportstops_stops.version";
    ...
    if (permission == LocationPermission.deniedForever) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(t.tools.nearbypublictransportstops.error
    ...
    if (!initDialogAlreadyShown) {
      showOkTextDialog(
          ...
          t.tools.nearbypublictransportstops
              .initial_dialog_message(source: "Opendata.ch (Transport)"));
      var prefs = await SharedPreferences.getInstance();
      prefs.setBool(
          SHARED_PREFERENCES_TOOL_NEARBYPUBLICTRANSPORTSTOPS_FIRSTDIALOGSHOWN,
          true);
  ...
  void showDataLicenseDialog() {
    List<TextButton> buttons = [
      TextButton(
        child:
            Text(t.tools.nearbypublictransportstops.data_license_dialog_open),
        onPressed: () {
          launchUrlInBrowser("https://transport.opendata.ch");
        },
      ),
      TextButton(
  ...
  Future<void> initPublicTransportNearbyStops() async {
    ...
    if (response.statusCode == 200) {
      ...
      errorMessage = t.tools.nearbypublictransportstops.error
          .api_error_xxx(errorCode: response.statusCode);
    }
  }
      ...
      if (response.body != "") {
        ...
        if (json != "") {
          ...
          NearbyPublicTransportStopsStation station =
              NearbyPublicTransportStopsStation.fromJson(
                  decodedJson["station"]);
          ...
          for (var board in stationBoard) {
            stationBoardList
                .add(NearbyPublicTransportStopsStationboard.fromJson(board));
          }
          if (mounted) {
            hideLoadingDialog(context);
          ...
          child: AlertDialog(
            content: Column(
              ...
              children: [
                ...
                Text(t.tools.nearbypublictransportstops.loading,
            ...
            content: SizedBox(
              ...
              child: ConstrainedBox(
                ...
                child: SingleChildScrollView(
                  ...
                  child: stationBoardList.isEmpty
                      ? Center(
                          child: Text(
                                ...
                                children: [
                                  ...
                                  if (board.stop?.departure != null)
                                    SizedBox(
                                      width: double.maxFinite,
                                      child: Text(
                                          "${t.tools.nearbypublictransportstops.departure} ${formatTimeStamp(board.stop?.departureTimestamp ?? 0, true)}",
                                          textAlign: TextAlign.left),
                                    ),
                                  if (board.stop?.platform != null &&
                                    ...
                                    SizedBox(
                                        width: double.maxFinite,
                                        child: Text(
                                            "${t.tools.nearbypublictransportstops.platform} ${board.stop?.platform ?? ""}",
                                            textAlign: TextAlign.left)),
                                  const Divider(),
                                ],
                              ),
                        ...
                        for (int i = 0; i < board.passList!.length; i++)
                          Column(
                            ...
                            children: [
                              ...
                              if (board.passList?[i].delay != null &&
                                  board.passList![i].delay! > 1)
                                SizedBox(
                                  width: double.maxFinite,
                                  child: Text(
                              ...
                              if (board.passList?[i].arrival != null)
                                SizedBox(
                                  width: double.maxFinite,
                                  child: Text(
                                      "${t.tools.nearbypublictransportstops.arrival} ${formatTimeStamp(board.passList?[i].arrivalTimestamp ?? 0, false)}",
                                      textAlign: TextAlign.left),
                                ),
                              if (board.passList?[i].departure != null)
                                SizedBox(
                                  width: double.maxFinite,
                                  child: Text(
                                      "${t.tools.nearbypublictransportstops.departure} ${formatTimeStamp(board.passList?[i].departureTimestamp ?? 0, false)}",
                                      textAlign: TextAlign.left),
                                ),
                              if (board.passList?[i].platform != null &&
                                ...
                                SizedBox(
                                    width: double.maxFinite,
                                    child: Text(
                                        "${t.tools.nearbypublictransportstops.platform} ${board.passList?[i].platform ?? ""}",
                                        textAlign: TextAlign.left)),
                              const Divider(),
                            ],
                          ),
